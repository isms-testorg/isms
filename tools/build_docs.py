...
FIELD_LABELS = {
    "en": {"id": "ID", "version": "Version", "status": "Status", "owner": "Owner",
           "approver": "Approver", "approved_on": "Approved", "next_review": "Next review",
           "classification": "Classification", "controls": "Annex A controls"},
    "de": {"id": "ID", "version": "Version", "status": "Status", "owner": "Verantwortlich",
           "approver": "Genehmigt von", "approved_on": "Genehmigt am", "next_review": "Nächste Überprüfung",
...
def doc_header_table(meta: dict, lang: str) -> list[str]:
...
def build_pack(lang: str, build: str, version: str) -> str:
    ...
    for section, path in sources:
        ...
        lines += ["", f"## {title}", ""]
        if meta:
            lines += doc_header_table(meta, lang)
        lines += [body.strip(), ""]
...
def main() -> int:
    ...
    for lang in LANGS:
        pack = build_pack(lang, build, args.version)
        for fmt in formats:
            if fmt != "md":
                pandoc(pack, fmt)

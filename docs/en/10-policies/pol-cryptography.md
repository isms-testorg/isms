...
Cryptography is selected according to information classification, risk, contractual needs, and applicable law. TLS protects data in transit; managed encryption protects restricted data at rest where supported. Keys, tokens, certificates, and recovery material are stored in the approved secret manager, separated from encrypted data, access-controlled, rotated on defined triggers, and revoked when compromised or no longer needed. Weak or obsolete algorithms are not introduced.
